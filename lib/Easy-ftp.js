...
EasyFTP.prototype.download = function(remotePath, localPath, cb, isRecursive){
	...
	else
	{
		...
		function ignoreDownloadFile(remotePath) {
			if (self.config.ignoreFilesEndWith) {
				for (let i of self.config.ignoreFilesEndWith) {
					if (remotePath.endsWith(i)) {
						return true
					}
				}
			}
		...
		function bodyFile(){
			if(self.isFTP)
			{
				if (ignoreDownloadFile(remotePath)) {
					self.emit("download", 'ignored: ' + localPath);
					self.cd(cwd, function(){
						if(cb)cb(undefined);
					});
				} else {
